...
| `--apply-fixes` | Beta | Interactively apply **safe inline** auto-fixes. Multi-line or ambiguous fixes stay as suggestions for manual review. |
...
> **Note:** Beta features work for common cases but have known edge-case gaps. Fixes generated by `--apply-fixes` should still be reviewed before committing.

`--experimental-js-ast` now runs a zero-dependency structural engine for JavaScript and TypeScript before merging any remaining coverage from the standard JS engine. It now covers multiline sink calls, split assignments, React / JSX `dangerouslySetInnerHTML` flows, object-literal and decorator/file-based route/auth definitions (including Fastify, Koa-style ambient middleware, Nest decorators, and Next route files), helper-call sink resolution, helper return-value propagation across local/imported JS/TS call chains, and cached relative-import JS/TS module-graph flow for common redirect / path / SSRF / route-access patterns, but it should still be treated as beta.
...
| Cross-file taint (inter-proc) | Partial (global graph v1) | ✅ | ✅ | ❌ |
| SARIF output | ✅ | ✅ | ✅ | ❌ |
| SBOM generation | ✅ | ❌ | ❌ | ❌ |
...
- **Regex-based detectors** may miss multi-line, obfuscated, or dynamically constructed sinks.
- **JavaScript support** is primarily pattern-matching; full AST taint requires the `structural` backend.
- **Cross-file taint** (global graph) is best-effort in v1; returns vs. function arguments are not fully modelled.
- **Minified code** produces unreliable line numbers and may generate false positives.
- **Template engines** (Jinja2, Handlebars) are partially supported; server-side template injection may be missed.
- **Per-file timeouts** default to 30 s; extremely large generated files may time out silently.
...
| [`js_ast_analyzer.py`](src/ansede_static/js_ast_analyzer.py) | Experimental syntax-aware JS/TS engine with zero-dependency structural call/property parsing plus fallback coverage from the standard JS analyzer |
