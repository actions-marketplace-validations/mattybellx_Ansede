...
function pushFindingDiagnostics(document, result, minOrder, diagnostics) {
...
async function scanDocument(document, collection, statusItem) {
    ...
    try {
        ...
        const totalFindings = (0, protocol_1.countFindings)(report);
...
function activate(context) {
    ...
    const scan = (document) => scanDocument(document, collection, statusItem);
    ...
    context.subscriptions.push(collection, statusItem, vscode.languages.registerCodeActionsProvider([{ scheme: 'file', language: 'python' }, { scheme: 'file', language: 'javascript' }], new AnsedeCodeActionProvider(), { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }), vscode.workspace.onDidOpenTextDocument(document => {
        ...
        if (editor && vscode.workspace.getConfiguration('ansede').get('scanOnOpen', true)) {
        ...
        for (const folder of folders) {
            const files = await vscode.workspace.findFiles(new vscode.RelativePattern(folder, '**/*.[pj]s'), '**/node_modules/**');
