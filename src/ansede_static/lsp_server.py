"""ansede_static.lsp_server
──────────────────────────────────────────────────────────────────────────────
Minimal Language Server Protocol (LSP 3.17) implementation for ansede-static.
...
Usage (from CLI):
  ansede-static --lsp          # starts the stdio server
"""
...
class LspServer:
    """
    Minimal ansede-static LSP server.
...
def run_lsp_server() -> None:
    """
    Start the ansede-static LSP server on stdio.

    This is the entry point invoked by ``ansede-static --lsp``.
    Logging is directed to stderr so as not to pollute the LSP wire format.
    """
    logging.basicConfig(
        level=logging.WARNING,
        stream=sys.stderr,
        format="ansede-lsp %(levelname)s: %(message)s",
    )
    LspServer().serve()
